...
case class Metrika(
  ...
  def call(method: String, url: String, params: Seq[(String, String)] = Seq(), bodyparams: JsValue = JsNull): JsValue = {
    val request = WS.url(url).withQueryString(("pretty", "1") +: ("oauth_token", token) +: params: _*)
    ...
    Await.result(fres, Duration.Inf).json
  }
